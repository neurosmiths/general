function [xPoints, yPoints] = plotSpikeRasterCats(spikes,cats,cols,varargin)
...
%
%       cats input is an M X 1 array of integer-formatted caegory labels
%
%       cols input is an P X 3 array of desired colors, where P =
%       length(unique(cats)) 
%
%   Output:
...
%% ~~~ UPDATES TO SORT BY CATEGORY ~~~~~~
%% AUTHOR    : Elliot Smith
%% $DATE     : 20220711
%% $Revision : 1.2 $
%% DEVELOPED : R2018b
%% FILENAME  : plotSpikeRasterCats.m


%% sort category labels and retain sorted indices.
[~,sortedIdcs] = sort(cats);

% arranging color arrays (IN ORIGINAL TRIAL ORDER)
for cts = 1:length(cats)
    LineFormat.Color(cts,:) = cols(cats(cts),:);
    MarkerFormat.Color(cts,:) = cols(cats(cts),:);
end
...
if islogical(spikes)
    ...
    switch plotType
        ...
        case 'vertline'
            ...
            xPoints = xPoints(:);
            yPoints = yPoints(:);
            plot(xPoints,yPoints,lineFormat{:});
        case 'horzline2'
    ...
    if strcmpi(plotType,'vertline') || strcmpi(plotType,'horzline')
        ...
        if strcmpi(plotType,'vertline')
            ...
            for trials = 1:nTrials
                ...
                % plotting per trial for vertlines
                plot(trialXPoints, trialYPoints,'Color',LineFormat.Color(sortedIdcs(trials),:));
            end
